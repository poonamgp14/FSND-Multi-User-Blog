...
class handler(webapp2.RequestHandler):
    ...
    def setCookie(self, username):
        ...
        self.response.headers.add_header(
            'Set-Cookie', 'user=%s' % str(securedUserName))
...
class SignUp(handler):
    ...
    def post(self):
        ...
        if self.name and self.email and self.pw:
            ...
            query = BlogDB.Users.query(ndb.AND(BlogDB.Users.name == self.name,
                                               BlogDB.Users.email == self.email
                                               )).fetch()
            if query:
                ...
                if self.pw == self.pwVerify:
                    ...
                    self.render("signup.html",
                                error='Both of the passwords should be same')
...
class LogIn(handler):
    ...
    def post(self):
        ...
        if self.loginName and self.loginpw:
            ...
            if query:
                if not self.validPw(self.loginName, self.loginpw,
                                    query[0].password):
                    self.render("login.html",
                                error='Username and/or password donot match'
                                )
                else:
        ...
        if not self.checkUser:
            ...
            if self.posts:
                for self.post in self.posts:
                    self.eachPost = {'user': self.post.username,
                                     ...
                                     'id': self.post.key.id(),
                                     'isLikable': self.post.isLikable
                                     if self.post.username == name or
                                     name in self.post.likedBy else True,
                                     'isUnlikable': self.post.isUnlikable
                                     if self.post.username == name or
                                     name in self.post.unlikedBy else True,
                                     'isEditable': self.post.isEditable
                                     if self.post.username == name else False,
                                     'isDeletable': self.post.isDeletable
                                     if self.post.username == name else False
                                     }
                    blogsList.append(self.eachPost)
                self.render(
                    "blogPost.html", blogsList=blogsList, user={'name': name})
...
class NewBlog(handler):
    ...
    def get(self):
        ...
        if not self.checkUser:
            ...
            name = self.checkUser
            commentResults = BlogDB.Comments.query(
                ndb.AND(BlogDB.Comments.postId == url[2][6:])).fetch()
            self.render("blogPostComment.html", blogsList=[
                {
                    'user': newBlog.username,
                        ...
                        'id': newBlog.key.id(),
                        'isLikable': newBlog.isLikable
                        if newBlog.username == name or
                        name in newBlog.likedBy else True,
                        'isUnlikable': newBlog.isUnlikable
                        if newBlog.username == name or
                        name in newBlog.unlikedBy else True,
                        'isEditable': newBlog.isEditable
                        if newBlog.username == name else False,
                        'isDeletable': newBlog.isDeletable
                        if newBlog.username == name else False
                        }],
                        user={'name': name},
                        previousComments=[
    ...
    def post(self):
        ...
        if not self.user:
            ...
            if self.ifContentOrComment:
                ...
                if blogToUpdate.username == self.user:
...
class LikedPost(handler):
    ...
    def post(self):
        ...
        if not self.user:
            ...
            ifAlreadyLiked = blogToUpdate.likedBy.get(self.user, 'None')
            if ((blogToUpdate.username != self.user) and
                    (ifAlreadyLiked == 'None')):
                print("I am here!")
...
class UnlikedPost(handler):
    ...
    def post(self):
        ...
        if not self.user:
            ...
            ifAlreadyUnliked = blogToUpdate.unlikedBy.get(self.user, 'None')
            if ((blogToUpdate.username != self.user) and
                    (ifAlreadyUnliked == 'None')):
...
class DeleteComment(handler):
    ...
    def post(self):
        ...
        if not self.user:
            ...
            if commentToUpdate.author == self.user:
